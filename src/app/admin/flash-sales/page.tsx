...
export default function AdminFlashSalesPage() {
  ...
  async function load() {
    try {
      ...
      // Flatten catalog - the API returns { services: [], platforms: [], settings: {} }
      const allServices: any[] = [];
      
      if (catalogJson.services && Array.isArray(catalogJson.services)) {
        // Services are already flattened in the catalog response
        catalogJson.services.forEach((service: any) => {
          allServices.push({
            id: service.id,
            name: service.name,
      ...
      } else if (catalogJson.platforms && Array.isArray(catalogJson.platforms)) {
        // Fallback: flatten from platforms structure
